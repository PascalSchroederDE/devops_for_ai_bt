...
Probably the most important point is a collaboration platform for the data so that an entire team can work on them at the same time as it is already common with code. Additional versioning of the data would be welcomed as well.
...
For a proper evaluation of the resulting model, good visualization of the testing results is desirable. The developer has to be able to see the results and react accordingly. A configurable, automated decision system would be optimal so that the developer can adjust the threshold above which the model will be deployed. If the result is below this threshold, it will either be rejected or adjusted with different parameters.
